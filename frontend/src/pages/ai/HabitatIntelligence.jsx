...
import { Leaf, Sparkles, HelpCircle } from 'lucide-react';
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div>
        ...
        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white mt-1">
          Habitat Intelligence System
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Monitor forest densities, land cover shifts, and climate degradation indicators using satellite imagery.
        </p>
      </div>
      ...
      <div className="glass-card p-8 min-h-[400px] flex flex-col items-center justify-center text-center relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-slate-50/90 dark:bg-slate-950/80 backdrop-blur-xs flex flex-col items-center justify-center p-6 z-10">
          <Leaf className="h-12 w-12 text-emerald-500 mb-4 animate-bounce" />
          ...
          <h4 className="text-base font-bold text-slate-800 dark:text-slate-200 mt-3">AI Habitat Intelligence</h4>
          <p className="text-xs text-slate-500 dark:text-slate-500 max-w-sm mt-2 leading-relaxed">
            This module is scheduled for development in Milestone 3. Remote sensing NDVIs and vegetation filters will report land degradation levels.
          </p>
        </div>

        <div className="space-y-3">
          <HelpCircle className="h-10 w-10 text-slate-500 mx-auto" />
          <h4 className="text-sm font-bold text-slate-300">Awaiting Satellite Layers</h4>
        </div>
      </div>
    </div>
  );
