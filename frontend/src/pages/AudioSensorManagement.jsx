...
import { PLACEHOLDER_AUDIO_NAME, formatISTDate } from '../utils/india';
...
const AudioSensorManagement = () => {
  ...
  useEffect(() => {
    fetchSensorsAndSites();
  }, []);

  const fetchSensorsAndSites = async () => {
    try {
      const [sensorsRes, sitesRes] = await Promise.all([
  ...
  const handleOpenEdit = (sensor) => {
    ...
    setSerialNumber(sensor.sensor_id || '');
    setStatus(sensor.status || 'Active');
    setBatteryLevel(sensor.battery_level ? sensor.battery_level.toString() : '100');
    setMonitoringSiteId(sensor.location_id ? sensor.location_id.toString() : '');
    setLatitude(sensor.latitude ? sensor.latitude.toString() : '');
    setLongitude(sensor.longitude ? sensor.longitude.toString() : '');
    setInstallationDate(sensor.installation_date || '');
    setFormErrors({});
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!serialNumber.trim()) errors.serialNumber = "Sensor Serial ID is required";
    ...
    try {
      if (isEditing) {
        await api.put(`/api/audio-sensors/${currentSensorId}`, payload);
        setToastMessage(`Audio sensor '${name}' updated successfully`);
      } else {
        await api.post('/api/audio-sensors', payload);
        setToastMessage(`Audio sensor '${name}' registered successfully`);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await api.delete(`/api/audio-sensors/${deletingId}`);
      setToastType('success');
      setToastMessage('Audio sensor hardware node deleted successfully');
      setTimeout(() => setToastMessage(null), 4000);
  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Acoustic Audio Sensors</h1>
          <p className="text-slate-550 dark:text-slate-400 text-sm mt-1">Configure decibel bioacoustic recorder devices deployed in monitoring zones.</p>
        </div>
        {canEdit && (
          <button
      ...
      {loading ? (
        ...
        viewMode === 'grid' ? (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {currentSensors.map((sensor) => (
              <div key={sensor.id} className="glass-card p-6 flex flex-col justify-between space-y-5 hover:border-emerald-500/25 transition-all shadow-sm">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className={`inline-flex px-2 py-0.5 rounded text-4xs font-bold border uppercase tracking-wider ${
                      sensor.status === 'Active' 
                        ...
                        : 'bg-slate-55 dark:bg-slate-950 text-slate-655 dark:text-slate-400 border-slate-205 dark:border-slate-800'
                    }`}>
                      {sensor.status}
                    </span>
                    {canEdit && (
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => handleDeleteTrigger(sensor.id)}
                  ...
                  <h3 className="text-base font-extrabold text-slate-900 dark:text-white line-clamp-1">{sensor.name}</h3>
                  <p className="text-4xs text-slate-500 dark:text-slate-500 font-mono mt-1">Model: {sensor.model} | S/N: {sensor.sensor_id}</p>
                  ...
                  <div className="mt-4 space-y-2 text-xs text-slate-555 dark:text-slate-400 font-semibold">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate">Siting base: {getSiteName(sensor.location_id)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Battery className={`h-4 w-4 shrink-0 ${getBatteryIconColor(sensor.battery_level)}`} />
                      <span>Battery Charge: {sensor.battery_level}%</span>
                    </div>
                    <div className="text-4xs text-slate-500 mt-1.5 font-mono">
                      GPS: {sensor.latitude?.toFixed(4)}, {sensor.longitude?.toFixed(4)} | Installed: {sensor.installation_date}
                    </div>
                  </div>
                </div>

                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-4xs text-slate-505 dark:text-slate-500 font-mono">
                  <span>Sensor ID: SNR-{sensor.id}</span>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
                ...
                <tbody className="divide-y divide-slate-200/50 dark:divide-slate-800/60 font-semibold">
                  {currentSensors.map((sensor) => (
                    <tr key={sensor.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                      <td className="px-5 py-4 font-bold text-slate-900 dark:text-white">{sensor.name} ({sensor.model})</td>
                      <td className="px-5 py-4 font-mono text-2xs">{sensor.sensor_id}</td>
                      <td className="px-5 py-4 text-slate-600 dark:text-slate-405">{getSiteName(sensor.location_id)}</td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 font-mono text-2xs">
                          <Battery className={`h-4.5 w-4.5 ${getBatteryIconColor(sensor.battery_level)}`} />
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 dark:bg-slate-900/10 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
          <Volume2 className="h-12 w-12 text-slate-400 dark:text-slate-650 mb-3" />
          <p className="text-sm font-bold text-slate-900 dark:text-slate-350">No Acoustic Sensors Registered</p>
          <p className="text-xs text-slate-500 dark:text-slate-500 mt-1 text-center max-w-xs leading-normal">
            No bioacoustic audio sensors match the filters. Click below to add a new hardware sensor.
          </p>
          {canEdit && (
            <button
              onClick={handleOpenAdd}
      ...
      {totalPages > 1 && !loading && (
        <div className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-900/10 rounded-xl border border-slate-200 dark:border-slate-950 text-xs font-semibold">
          <span className="text-slate-500 dark:text-slate-400">
            ...
            of <strong className="text-slate-800 dark:text-slate-200">{filteredSensors.length}</strong> sensors
          </span>
            ...
            <div className="enterprise-modal-header">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                  {isEditing ? 'Edit Audio Sensor' : 'Add Audio Sensor to Registry'}
                </h3>
              </div>
              <button
              ...
              <div className="enterprise-modal-body">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="enterprise-label">Sensor Name *</label>
                    <input
                      type="text"
                      required
                      value={name}
                      ...
                      placeholder={PLACEHOLDER_AUDIO_NAME}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Song Meter SM4"
                    />
                    {formErrors.model && <p className="text-rose-500 text-4xs font-bold mt-1">{formErrors.model}</p>}
                  </div>

                  <div>
                    <label className="enterprise-label">Sensor Serial ID *</label>
                    <input
                      type="text"
                      required
                      value={serialNumber}
      ...
      <ConfirmModal
        ...
        message="This action will permanently delete this hardware sensor registration and clear its deployed status logs. This action cannot be undone."
...
export default AudioSensorManagement;
